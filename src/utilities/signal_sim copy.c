...
#define BASE_NOISE  50            // 基底噪声幅值
#define ECHO_STEP   80.0          // 回波间隔步长（采样点数）
#define ECHO_WIDTH  10.0          // 回波包络半宽度（控制回波宽度，越大越宽）
#define ECHO_AMPLITUDE 1500.0     // 回波幅度（最大值，后续每个回波递减）
#define ECHO_AMP_STEP 50.0        // 回波幅度递减步长（每个回波比前一个小多少）
...
int main() {
    // 1. 定义缺陷回波参数
    // EchoParams echos[] = {
    //     {1000.0f, 1500.0f, 30.0f},
    //     {2000.0f, 1300.0f, 30.0f},
    //     {3000.0f, 1000.0f, 30.0f},
    //     {4000.0f,  600.0f, 30.0f}
    // };
    ...
    for (int i = 0; i < TOTAL_POINTS; i++) {
        ...
        for (int e = 0; e < echo_count; e++) {
            float x = (float)i - i* ECHO_STEP; // echos[e].center;
            // 高斯包络：exp(-x^2 / width^2)
            // float envelope = expf(-(x * x) / (echos[e].width * echos[e].width));
            float envelope = expf(-(x * x) / (ECHO_WIDTH * ECHO_WIDTH));
            // 载波：cos(2πf t + φ)，与中心频率同频
            float echo_signal = (ECHO_AMPLITUDE - i* ECHO_AMP_STEP) * envelope * sinf(2.0f * PI * CARRIER_FREQ * t);
            signal += echo_signal;
        }
    ...
    printf("缺陷回波中心位置: ");
    for (int e = 0; e < echo_count; e++) {
        printf("%.0f ", e * ECHO_STEP); //echos[e].center);
    }
    printf("\n缺陷回波幅度: ");
    for (int e = 0; e < echo_count; e++) {
        printf("%.0f ", ECHO_AMPLITUDE - e * ECHO_AMP_STEP); //echos[e].amplitude);
    }
